...
**測試日期**：2026-05-27**測試範疇**：護理推車即時雙向翻譯系統 — ASR Provider 比較**測試語言**：中文（zh）→ 英文（en）**測試句子**：10 句護理場景對話（zh-01 至 zh-10）
...
1. **gpt-realtime-whisper**（GA API）：OpenAI 最新串流轉錄模型，使用 `wss://api.openai.com/v1/realtime?intent=transcription`

1. **gpt-realtime-translate**：OpenAI Realtime API 直接翻譯模式

1. **Deepgram Nova-3**：第三方串流 ASR 服務

1. **Gladia Solaria-1**：第三方串流 ASR 服務
...
1. 後處理：使用 OpenCC 或 GPT 將簡體轉繁體

1. 搭配 gpt-4.1-mini 進行翻譯（預計總延遲 1,612 + ~500ms = ~2,100ms）
...
**預估總延遲**：688ms（首字）+ ~500ms（翻譯）= **~1,200ms****優點**：即時串流、準確度高（修正後）、成本合理
...
**預估總延遲**：665ms（翻譯首字）**優點**：最快，但成本較高（兩個 WebSocket 連線）

/**
 * 使用真實語音的 OpenAI Realtime API 診斷測試
 * 使用 FFmpeg 將 MP3 轉換為 PCM16 24kHz 格式
 */
...
/**
 * 使用 FFmpeg 將音訊檔案轉換為 PCM16 24kHz mono little-endian
 */
...
async function runDiagnostic() {
    console.log('═══════════════════════════════════════════════════════════');
    console.log('    OpenAI Realtime API 語音診斷測試');
    console.log('═══════════════════════════════════════════════════════════\n');
    ...
    // 轉換音訊
    console.log('🔄 轉換音訊為 PCM16 24kHz...');
    let pcmBuffer: Buffer;
    try {
        pcmBuffer = await convertToPCM16(testAudioFile);
        const durationMs = (pcmBuffer.length / 2 / 24000) * 1000;
    ...
    try {
        ...
        const chunkSize = 24000 * 2 * 0.1; // 100ms @ 24kHz mono 16-bit = 4800 bytes
        ...
        // 等待 VAD 處理
        console.log('⏳ 等待 VAD 處理...');
        await new Promise(resolve => setTimeout(resolve, 3000));

        // Server VAD 模式下應該自動 commit，但我們也手動試試
        if (!events.some(e => e.type === 'speech_stopped')) {
            console.log('📤 VAD 未觸發，手動 commit...');
        ...
        console.log(`  ${hasSpeechStarted ? '✅' : '❌'} speech_started (VAD 偵測到語音開始)`);
        console.log(`  ${hasSpeechStopped ? '✅' : '❌'} speech_stopped (VAD 偵測到語音結束)`);
        ...
        if (!hasSpeechStarted) {
            console.log('');
            console.log('⚠️ VAD 沒有偵測到語音，可能原因：');
            console.log('   1. 音訊格式轉換有問題');
            console.log('   2. VAD 敏感度設定太高');
            console.log('   3. 音訊太短或音量太低');
        }
